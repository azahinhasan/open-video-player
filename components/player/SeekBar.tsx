...
export function SeekBar({
  ...
  // below corrects it.
  const progressFraction = useSharedValue(
  ...
  useAnimatedReaction(
    () =>
      duration > 0
        ? Math.min(
            PREVIEW_BUCKETS - 1,
            Math.floor(dragFraction.value * PREVIEW_BUCKETS),
          )
        : -1,
    (bucket, previous) => {
      if (bucket !== previous) {
  ...
  useEffect(() => {
    ...
    generateSeekPreviewThumbnail(
      videoUri,
      videoId,
      previewBucket,
      Math.floor(timeSeconds * 1000),
    ).then((uri) => {
  ...
  // Only dispatches the seek — the display update (progressFraction) is
  // written separately, directly inside each gesture's own worklet below,
  // not here. commitSeek only runs after a runOnJS round-trip to the JS
  // thread, but isDragging flips back to false synchronously on the UI
  // thread (via onFinalize, in the very same tick the gesture ends) — if
  // progressFraction were only updated once commitSeek actually executes,
  // there'd be a brief window where isDragging is already false but
  // progressFraction still holds the pre-seek value, visible as the bar
  // snapping back to the old position before jumping forward to the new
  // one. Writing progressFraction synchronously, in the same worklet tick
  // as the isDragging flip, closes that gap entirely.
  const commitSeek = useCallback(
    (fraction: number) => {
      onSeek(Math.min(1, Math.max(0, fraction)) * duration);
    },
    [duration, onSeek],
  );
    ...
    () =>
      Gesture.Pan()
        ...
        .onEnd((_event, success) => {
          ...
          // See commitSeek's comment above for why this is written here,
          // synchronously, rather than inside commitSeek itself.
          progressFraction.value = dragFraction.value;
    ...
    () =>
      Gesture.Tap()
        ...
        .onEnd((event, success) => {
          ...
          const width = railWidth.value || 1;
          const fraction = Math.min(1, Math.max(0, event.x / width));
          // See commitSeek's comment above — written synchronously here so
          // it lands in the same tick panGesture's onFinalize flips
          // isDragging back to false (tapGesture winning the race doesn't
          // stop that onFinalize from still firing).
  ...
  const seekGesture = useMemo(() => {
    // console.log("SeekBar: creating seekGesture", tapGesture);
  ...
  return (
    <View style={styles.wrapper}>
      <GestureDetector gesture={seekGesture}>
        ...
        >
          <Animated.View
            style={[styles.previewBubble, previewStyle]}
            pointerEvents="none"
          >
          ...
          {buffering ? (
            <Animated.View
              style={[styles.bufferingDot, playheadStyle]}
              pointerEvents="none"
            />
          ) : null}
        </View>
