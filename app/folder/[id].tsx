...
export default function FolderScreen() {
  ...
  const [propertiesVideo, setPropertiesVideo] = useState<VideoAsset | null>(
    null,
  );
  ...
  const videos = useMemo(
    () => sortVideos(videosForFolder(id), sortMode),
    [videosForFolder, id, sortMode],
  );
  ...
  const handleConfirmRename = useCallback(
    async (newBaseName: string) => {
      ...
      setRenameVideo(null);
      const result = await renameVideoAsync(
        target.id,
        target.filename,
        newBaseName,
      );
      if (result) {
        updateVideoMeta(target.id, {
  ...
  const handleConfirmVault = useCallback(async () => {
    ...
    if (committedCount > 0) {
      useToastStore.getState().show("Moved to Vault");
    } else if (failedCount > 0) {
      Alert.alert(
        "Couldn't move to Vault",
        "The video was not moved. Please try again.",
      );
    }
  ...
  const handleConfirmBatchVault = useCallback(async () => {
    ...
    if (committedCount > 0) {
      useToastStore
        .getState()
        .show(
          `${committedCount} video${committedCount === 1 ? "" : "s"} moved to Vault`,
        );
    }
    if (failedCount > 0) {
      Alert.alert(
  ...
  const propertyRows: PropertyRow[] = useMemo(() => {
    ...
    return [
      ...
      { label: "Folder", value: folder?.name ?? "Unknown" },
      {
        label: "Duration",
        value:
          propertiesVideo.duration !== null
            ? formatTime(propertiesVideo.duration)
            : "Unknown",
      },
      {
        label: "Resolution",
